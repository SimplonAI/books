...
class RecFactory:
    ...
    def __init__(self, ratings: pd.DataFrame, logger=None):
        if logger is not None:
            ...
            ), "Votre classe Logger n'implémente pas une fonction debug"
        self._logger = logger
        print(ratings)
    ...
    def _reindex(self, ratings: pd.DataFrame, inplace=False):
        ...
        rt["user_id"] = (
            rt["user_id"]
            .map(dict(zip(self._user_ids_idx.values(), self._user_ids_idx.keys())))
            .astype(int)
        )
        rt["book_id"] = (
            rt["book_id"]
            .map(dict(zip(self._book_ids_idx.values(), self._book_ids_idx.keys())))
            .astype(int)
        )
        return rt
    ...
    def build_rating_sparse_tensor(self, ratings_df: pd.DataFrame):
        ...
        return tf.SparseTensor(
            ...
            values=values,
            dense_shape=[
                self.ratings["user_id"].drop_duplicates().shape[0],
                self.ratings["book_id"].drop_duplicates().shape[0],
            ],
        )

    def sparse_mean_square_error(
        self, sparse_ratings, user_embeddings, book_embeddings
    ):
    ...
    def compute_scores(
        ...
        query_embedding,
        item_embeddings,
        measure: ComputeMethodEnum = ComputeMethodEnum.DOT,
    ):
    ...
    def user_recommendations(
        ...
        if exclude_rated and user_id is not None:
            ...
            user_id = dict(zip(self._user_ids_idx.values(), self._user_ids_idx.keys()))[
                user_id
            ]
            rated_books = self.ratings[self.ratings.user_id == user_id][
                "book_id"
            ].values
